...
class AulaModel extends Model {
    ...
    public function selectAulaByTrilhaId($id){
        $list = [];
        $sql = "SELECT aula.id_aula, aula.id_secao, aula.nome, aula.descricao, aula.data, aula.imagem FROM aula JOIN secao ON secao.id_secao = aula.id_secao WHERE secao.id_trilha = :id ORDER BY aula.num_ordem";
